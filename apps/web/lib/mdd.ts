...
export const CATEGORIES: CategoryNode[] = [
  // ─── Women's Western Wear ──────────────────────────────────────────────
  {
    ...
    mandatoryAttrs: [
      ...COMMON_MANDATORY,
      'pattern', 'neck_collar', 'sleeve', 'dress_shape', 'dress_length',
    ],
...
export const LOV = {
  ...
  color_family: [
    'Black', 'White', 'Grey', 'Navy', 'Blue', 'Red', 'Pink', 'Orange',
  ...
  fabric_family: [
    'Cotton', 'Polyester', 'Polyester Blend', 'Viscose', 'Rayon', 'Linen',
...
export function classifyToL4(
  ...
  const scored = CATEGORIES.map((cat) => {
    ...
    // Partial Product Type match
    if (pt && cat.sellerSynonyms.some((s) => pt.includes(s) || s.includes(pt))) {
      score += 0.3;
